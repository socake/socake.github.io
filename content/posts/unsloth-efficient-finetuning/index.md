...
Unsloth 是那种"用过就回不去"的工具——只要你的场景合适（单卡 LoRA），它的体验远超原生路径。它的定位很清楚：**不是一个通用训练框架，而是一个极致优化的单卡 LoRA 加速器**。理解了这个定位，选型和用法就都清楚了。

不要因为它快就把所有场景都塞给它，也不要因为它在多卡上弱就看不上它。Unsloth 和 LLaMA Factory 在我心里是互补的两个工具，前者让单卡试错成本降到最低，后者让多卡生产流程标准化。两个都掌握，覆盖 95% 的微调需求。
