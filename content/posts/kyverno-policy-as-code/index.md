...
这篇文章是我在 Kyverno 1.12+ 上的生产落地经验。涵盖四种策略类型、CEL 表达式、PolicyException、性能调优、和其他工具的组合。读完你应该能自己从零搭一套完整的策略治理体系。
...
Policy as Code 听起来是个宏大的概念，但具体到 Kyverno 这个层面，它就是"**用声明式 YAML 表达集群该有的样子**"。每条策略都是一段工程化的表达，每条豁免都有可追溯的理由，每次违规都有可审计的记录。这是运维从"救火员"变成"平台工程师"最关键的能力之一。

当你的集群里跑着几百条 Kyverno 策略，每条都在 PR 里被 review 过、每条都有对应 metric、每条都有 exception 管理流程的时候，你会发现所谓的"安全治理"、"合规遵守"、"最佳实践落地"不再是靠口头提醒和定期培训，而是**被固化在代码里的自动化规则**。这就是 policy as code 的真正价值。

下一篇也是这个零信任系列的最后一篇：SLSA 软件供应链等级实施。那是把前面所有这些工具拧成一根绳子的终极框架。
