...
Flagger 不是银弹，它解决的只是"发布这个动作本身的风险摊薄"。它无法解决：

- **架构设计错误**：比如一个新功能从一开始设计就错，指标再好看也白搭。
- **需求错误**：产品经理要的功能本身有问题，发布成功也没用。
- **数据层变更**：数据库 schema 变更、数据迁移，Flagger 管不了。
- **跨服务事务**：涉及多个服务的原子变更，要么手动协调，要么用 feature flag。

理解它的边界之后，把它装好、用好、纳入 CI/CD，是最务实的工程化选择。渐进式交付的核心哲学只有一句话：**"不要相信发布之前的测试，要相信发布之中的指标。"**

所有的 Canary CR、MetricTemplate、Webhook，都是为这句话服务的。
