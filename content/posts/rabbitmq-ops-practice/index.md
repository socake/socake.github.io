...
RabbitMQ 在我们生产环境承载了 AI 任务调度、跨服务异步通知、工单流转等业务链路。相比 Kafka，RabbitMQ 的消息路由更灵活，运维模型也更复杂——Exchange 类型、队列持久化、ACK 机制、集群脑裂……每个坑都可能导致消息丢失或服务雪崩。这篇文章梳理了我在实际运维中积累的经验，从集群搭建到故障处理全面覆盖。
