...
AI 时代，GPU 是最贵的计算资源，也是最容易浪费的资源。一张 A100 每月云上租用成本超过 2000 美元，但很多团队的 GPU 利用率不到 30%。Kubernetes 提供了管理 GPU 的基础框架，但默认配置远远不够——你需要理解整个技术栈，才能真正把 GPU 用起来、用好。

这篇文章从底层驱动到上层调度策略，系统梳理在 K8s 上管理 GPU 资源的完整路径。
...
通过这一套完整的 GPU 基础设施方案，从单卡推理到多机多卡训练，Kubernetes 都能提供可靠的支撑。关键是把节点弹性（Karpenter）、资源隔离（MIG/时间片）、监控告警（DCGM）这三件事做扎实，剩下的调优工作才能有据可依。
