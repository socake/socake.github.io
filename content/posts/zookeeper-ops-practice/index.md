...
Zookeeper 是分布式系统中历史最悠久的协调服务之一，在 Kafka、HBase、Hadoop 生态中曾经无处不在。随着云原生的演进，它的使用场景逐渐收窄，但在现有存量系统的运维中依然是必须掌握的技能。这篇文章整理了实际运维 Zookeeper 集群的完整经验，从原理到实践，从部署到故障处理。
...
Zookeeper 是分布式系统工程的重要里程碑，理解它的原理对于设计可靠的分布式系统依然有价值，即使在实际生产中已经越来越少直接使用它。掌握好运维技能，同时也了解何时可以"毕业"，是一个成熟运维工程师的标志。
