...
2019 年 Docker 创始人 Solomon Hykes 发了一条推文："如果 WASM+WASI 在 2008 年就存在，我们就不需要创建 Docker 了。"这句话被引用烂了，但它点出了一个真实的工程问题：**容器解决的本质是进程隔离和运行时环境打包，而 Wasm 从字节码层面就内建了这两个能力。**

说清楚云原生 Wasm 之前，需要先把一个误解排掉：这里讨论的 Wasm 和前端的 Wasm 是同一个字节码格式，但运行场景完全不同。浏览器里的 Wasm 是为了把 C++/Rust 代码跑在 JS 引擎上；云原生里的 Wasm 是为了在服务端提供一个**比容器更轻、比进程更安全**的执行单元。
