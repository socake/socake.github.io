...
Terragrunt 不是"Terraform 的替代"，而是"Terraform 在规模化场景下的必备 wrapper"。它的价值不在于引入新概念，而在于把 Terraform 生态里已经存在的"最佳实践"自动化：

- DRY backend 和 provider 配置
- 跨 state 依赖声明
- 批量 plan/apply
- unit 模板化和 on-demand 生成

2025 年的 Stacks 特性是一个显著的进化，把 "multi-region 资源模板化" 这个长期痛点解掉了。如果你已经在用 Terraform 或 OpenTofu，state 超过 20 个，强烈建议评估 Terragrunt 1.0+。迁移成本不大（Terragrunt 是增量的，可以先套一个目录试），收益明显。
