...
这一篇是整个零信任系列里最"琐碎"的一篇——没有炫酷的 eBPF、没有花哨的身份协议，只有密钥管理这件枯燥但致命的事。但做运维这几年我越来越觉得，**零信任的大部分价值其实就沉淀在这些枯燥的工程化细节里**。你可以有最时髦的 SPIRE、最强的 Cilium、最严格的 Falco，但只要有一个长期密码躺在某个 YAML 文件里，整个防御体系就有一个零成本的绕过路径。

自动化密钥管理不是加分项，是零信任的地基。下一篇我会写 Pod Security Standards 的落地，那是"工作负载本身能做什么"的底线防御。
